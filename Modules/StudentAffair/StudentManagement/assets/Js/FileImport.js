...
function createDifferentSheetWorkSection(sheetList, counter) {
    if (counter !== 1)
        $("#sheetNoId").append(` <a id="importExcelSheetTabID-${counter}" class="non-selected-tab-section tab-context-header">
                            ${sheetList} </a>`);
    else
        $("#sheetNoId").append(`  <a id="importExcelSheetTabID-${counter}" class="selected-tab-section tab-context-header">
                            ${sheetList}</a>`);
}
