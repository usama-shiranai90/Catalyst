...
select *
from clo
where courseCode = 'SEN-32';


select b.curriculumCode, curriculumYear
from section
...
where a2.questionCode;


select a2.studentRegCode, a.topic, a.totalMarks, a2.totalObtainedMarks
from assessment as a
...
order BY weeklyTopicCode desc limit 1;
...
from assessment as a
    join assessmentquestion a2
on a.assessmentCode = a2.assessmentCode
where sectionCode = 7
  and courseCode = 'SEN-32'
    limit 5;
...
where sectionCode = 7
  and courseCode = 'SEN-32'
order by assessmentCode desc limit 1;

select questionCode, c.cloName, detail, totalMarks
from assessment as a
         join assessmentquestion a2 on a.assessmentCode = a2.assessmentCode
         join clo c on a2.cloCode = c.CLOCode
where sectionCode = 7
  and a.courseCode = 'SEN-32'
  and a.assessmentCode = 63;
...
from batch
where year between
    (select YEAR (DATE_SUB((select dateCreated from batch order by dateCreated desc limit 1)
    , INTERVAL 5 year )))
  and
    (select year from batch order by year desc limit 1)
...
from courseregistration
         inner join registeredcourses r on courseregistration.courseRegistrationCode = r.courseRegistrationCode
         inner join course c on r.courseCode = c.courseCode
where courseregistration.studentRegCode = 'FUI/FURC-SP-18-BCSE-040'
  and courseregistration.semesterCode = 42;
...
from batch
    ...
    join section s2
on s.semesterCode = s2.semesterCode;
...
select semesterCode, batchCode, semesterName
from semester
group by batchCode desc;


-- TESTER FOR CPM:

select semesterCode, facultyCode, courseCode, f.sectionCode, sectionName, isCoordinator
from section
...
where semesterCode = 10
  and courseCode = 'SEN-28';


select p.courseCode, courseTitle, creditHours, curriculumCode, preRequisiteName
from course
         join prerequisites p on course.courseCode = p.courseCode
where p.courseCode = 1;

select *
from courseprofile as cp
         inner join courseprofileassessmentinstruments c on cp.courseProfileCode = c.courseProfileCode
         inner join courseprofileinstructordetail c2 on cp.courseProfileCode = c2.courseProfileCode
where cp.courseProfileCode = 19
  and facultyCode;


#
Getting the latest eligible season for Program Manager
select *
from season
...
select batch.batchCode, semesterCode, semesterName
from batch
         JOIN semester s on batch.batchCode = s.batchCode
where batchName = 'FA15'
  AND programCode = 1
  and semesterName = 1;


INSERT INTO `clo` (`CLOCode`, `curriculumCode`, `programCode`, `batchCode`, `cloName`, `description`, `domain`,
                   `btLevel`, `courseCode`)
VALUES (1, 1, 1, 1, 'CLO-1', 'Handwriting', 'ABC', '4', 'SEN-28'),
